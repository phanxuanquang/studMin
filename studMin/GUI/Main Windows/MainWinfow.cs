...
namespace studMin
{
    public partial class MainWinfow : Form
    {
        ...
        private string Title(string user, string id, string purpose, string subject)
        {
            ...
            return String.Format("{0} | Mã định danh: {1} | Vai trò: {2}", user, id.Substring(0, 8).ToUpper(), purpose) + (String.IsNullOrEmpty(subject) == true ? string.Empty : " | Môn học phụ trách: " + subject);
        }
    }
}
